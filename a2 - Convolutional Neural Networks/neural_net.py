...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    z = np.dot(X, W1) + b1 # multiply and sum weights with input data, b1 represents bias term
    h1 = np.maximum(0, z) # ReLU activation function to introduce nonlinearity, negative values set to 0.
    # The activation function determines whether or not the 'neuron' was fired (activated)
    scores = np.dot(h1, W2) + b2 # determine scores with second set of weights and second bias term b2
    #############################################################################
    #                              END OF YOUR CODE                             #
    #############################################################################
    ...
    softmax_prob = exp_scores / np.sum(exp_scores, axis=1).reshape(N,1) # Obtain probability distribution of C classes
    ...
    dprob = softmax_prob
    dprob[np.arange(N),y] = y_prob - np.ones(N) # For only the true values of y, size (N, C)

    # backward pass
    grads['b2'] = np.sum(dprob,axis=0) / N # gradient of db2 is dL/db2 = SM(i) - 1, size (1, C)
    grads['W2'] = np.dot(h1.T, dprob) / N # gradient of dW2 is dL/dW2 = (SM(i) - 1)(ReLU(z)), size (H, C)

    db = np.dot(dprob, W2.T) # to compute b1, W1 gradient
    db[h1 <= 0] = 0     # the derivative of ReLU is the step function. for values > 0, derivative is 1 (binary mask)

    grads['b1'] = np.sum(db, axis=0) / N # gradient of db1 is dL/db1 = (SM(i) - 1)d(ReLU(z))(W2), size (1, H)
    grads['W1'] = np.dot(X.T, db) / N # gradient of dW1 is dL/dW1 = (SM(i) - 1)d(ReLU(z))(W2)(X), size(D, H)
    grads['W2'] += 2*reg*W2 # regularization from gradient of L2norm squared (2*reg*W)
    grads['W1'] += 2*reg*W1
